...
class Log(): 
    ...
    def logout(self, username): 
        self.fa.write('UPDATE Users SET logged_in = False WHERE username = "{un}"'.format(un=username)+'\n')
        self.fa.flush()
            
    def delete(self, username): 
        self.fa.write('DELETE FROM Users WHERE username = "{un}"'.format(un=username)+'\n')
        self.fa.flush()
        ...
        if mask & selectors.EVENT_READ: 
            ...
            if opcode == NEW_PRIMARY: 
                ...
                if self.db.is_logged_in(sentto): 
                    ...
                    print("Putting stuff in log: {uuid}, {sentto}, {sentfrom}, {msg}".format(uuid=uuid, sentto=sentto, sentfrom=sentfrom, msg=msg))
                    self.log.put_in_messages_queue(uuid, sentto, sentfrom, msg)
                if self.primary: 
